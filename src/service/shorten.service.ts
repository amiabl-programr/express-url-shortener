...
export async function createShortURL(originalUrl: string): Promise<string> {
  ...
  const existing = await client.query('SELECT short_url FROM urls WHERE long_url = $1', [originalUrl]);
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    await client.query('INSERT INTO urls (short_url, long_url) VALUES ($1, $2)', [shortUrl, originalUrl   ]);
    return  shortUrl;
  } else {
